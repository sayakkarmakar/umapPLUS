...
def smooth_knn_dist(distances, k, n_iter=64, local_connectivity=1.0, bandwidth=1.0):
    ...
    non_zero_dists = np.maximum(distances[:, 1:local_connectivity+1], 0.0)
    index = np.floor(local_connectivity).astype(int)
    interpolation = local_connectivity - index
    rho = np.where(non_zero_dists.shape[1] >= local_connectivity, 
                   non_zero_dists[np.arange(distances.shape[0]), index-1], 
                   np.max(non_zero_dists, axis=1))
    rho = np.where(index > 0, rho + interpolation * (non_zero_dists[np.arange(distances.shape[0]), index] - non_zero_dists[np.arange(distances.shape[0]), index-1]), rho + interpolation * non_zero_dists[:, 0])

    lo = np.zeros(distances.shape[0], dtype=np.float32)
    hi = np.full(distances.shape[0], np.inf, dtype=np.float32)
    mid = np.ones(distances.shape[0], dtype=np.float32)

    for n in range(n_iter):
        psum = np.sum(np.where(distances[:, 1:] - np.expand_dims(rho, axis=1) > 0, np.exp(-(distances[:, 1:] - np.expand_dims(rho, axis=1)) / np.expand_dims(mid, axis=1)), 1.0), axis=1)

        if np.all(np.fabs(psum - target) < SMOOTH_K_TOLERANCE):
            break

        lo = np.where(psum > target, lo, mid)
        hi = np.where(psum > target, mid, hi)
        mid = np.where(psum > target, (lo + hi) / 2.0, np.where(hi == np.inf, mid * 2, (lo + hi) / 2.0))

    result = mid

    result = np.where(rho > 0.0, np.where(result < MIN_K_DIST_SCALE * np.mean(distances, axis=1), MIN_K_DIST_SCALE * np.mean(distances, axis=1), result), np.where(result < MIN_K_DIST_SCALE * mean_distances, MIN_K_DIST_SCALE * mean_distances, result))
